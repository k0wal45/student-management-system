...
encode_salt(char* salt, u_int8_t* csalt, char minor, u_int16_t clen, u_int8_t logr)
{
	...
	// Max rounds are 31
	_snprintf_s(salt + 4, 4, _TRUNCATE, "%2.2u$", logr & 0x001F);
...
node_bcrypt(const char* key, size_t key_len, const char* salt, char* encrypted)
{
	...
	if (*salt > BCRYPT_VERSION) {
		...
		strcpy_s(encrypted, strlen(error) + 1, error);
	...
	Blowfish_expandstate(&state, csalt, salt_len, (u_int8_t*)key, (u_int16_t)key_len);
	...
	_snprintf_s(encrypted + i, 4, _TRUNCATE, "%2.2u$", logr & 0x001F);
